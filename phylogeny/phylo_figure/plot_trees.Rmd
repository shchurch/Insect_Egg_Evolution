...
rainford = dendropy.Tree.get(path='./rainford_backbone.tre', schema='nexus')
...
tips_to_rotate = list(c('Phasmatodea','Mantodea'),
                      c('Siphonaptera','Mecoptera'),
                      c('Megaloptera','Neuroptera'),
                      c('Coleoptera','Strepsiptera'),
                      c('Coleoptera','Neuroptera'),
...
```{r}
trees = list('Misof' = as.treedata(misof_backbone),
             'Rainford' = as.treedata(rainford_backbone))


tree_plots = lapply(trees,function(tree){
  ggtree(tree,ladderize = F) %>%  groupOTU(tax2orders) +
    aes(color=group) +
    theme_tree2() +
...
tree_plots = lapply(tree_plots, function(x)x + scale_x_continuous(labels=function(x){-x}, breaks=c(0,-100,-200,-300,-400,-500),limits = c(-500,450)))
...
Now we will plot our posterior distribution of trees. Let's first read them:

```{r}
misof_posterior = read.nexus(file = 'misof_100_posterior.trees')

rainford_posterior = read.nexus(file = 'rainford_100_posterior.trees') 
```

Now let's plot all posterior trees, in slanted format

```{r}
tree_plots = lapply(list(misof_posterior[1:50], rainford_posterior[1:50]),function(trees){
  p = ggtree(trees[[1]],
             ladderize = T, 
             open.angle = 3, 
             size = 0.1,
             layout='slanted',
             alpha=0.07) %>%  groupOTU(tax2genera) +
    aes(color=group) +
    theme_tree() +
    scale_color_manual(values = mrk) 
  ...
  for (i in 2:length(trees)){
    ...
    p = p + geom_tree(aes(color=group), 
                      size = 0.1, 
                      alpha=0.1,
                      layout='slanted',
                      data=(ggtree(trees[[i]])%>%groupOTU(tax2genera))$data 
                      )
  }
...
p = grid.arrange(tree_plots[[1]],tree_plots[[2]],nrow=1)
print(p)
ggsave(filename = 'posterior_slanted.pdf',plot = p, device = pdf,width = 6.5,height=9,useDingbats=F)

```

Let's plot them again, in rectangular format

```{r}
tree_plots = lapply(list(misof_posterior, rainford_posterior),function(trees){
  p = ggtree(trees[[1]],
             ladderize = T, 
             open.angle = 3, 
             size = 0.07,
             layout='rectangular',
             alpha=0.1) %>%  groupOTU(tax2genera) +
    aes(color=group) +
    theme_tree() +
    scale_color_manual(values = mrk) 
...
p = grid.arrange(tree_plots[[1]],tree_plots[[2]],nrow=1)
print(p)
ggsave(filename = 'posterior_rectangular.pdf',plot = p, device = pdf,width = 6.5,height=9,useDingbats=F)
